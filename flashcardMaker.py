...
def getFieldsFromMycoMatch(name, rank, myocMatchMap):
  if rank != "species":
     return None
...
def createNote(taxon_photo_paths, observation_data, my_model, my_deck, mycoMatchMap):
  try:
    ...
    mycoMatchFields = getFieldsFromMycoMatch(scientificName, observation_data['taxon']['rank'], mycoMatchMap)
    ...
    print("Created card for " + str(observation_data['id']))
  except Exception:
...
def run():
    try:
      ...
      while line != "":
        ...
        createNote(photo_paths, observation_data, model, deck, mycoMatchMap)
